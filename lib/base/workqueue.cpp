...
void WorkQueue::StatusTimerHandler()
{
	...
	if (pending > m_TaskStats.CalculateRate(Utility::GetTime(), 5) * 5) {
		timeInfo = " empty in ";
	...
	if (pending > 0 || m_StatusTimerTimeout < now) {
		Log(LogInformation, "WorkQueue")
			<< "#" << m_ID << " (" << m_Name << ") "
			<< "items: " << pending << ", "
			<< "rate: " << std::setw(2) << m_TaskStats.CalculateRate(Utility::GetTime(), 60) << "/s "
			<< "(" << m_TaskStats.CalculateRate(Utility::GetTime(), 60) * 60 << "/min " << m_TaskStats.CalculateRate(Utility::GetTime(), 60) * 60 * 5 << "/5min " << m_TaskStats.CalculateRate(Utility::GetTime(), 60) * 60 * 15 << "/15min);"
			<< timeInfo;
	}
